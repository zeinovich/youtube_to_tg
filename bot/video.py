...
PATH = 'C:/Users/nikit/youtube_to_tg'
class Video(YouTube):
    ...
    def download_thumb(self):
        ...
        with open(f'{PATH}/thumbnails/{self.video_id}.png', 'w') as f:
    ...
    def upload_beta(self, bot: TeleBot, chat_id: int, curs: Cursor):
        ...
        curs.insert(columns=['video_id', 'chat_id', 'date', 'only_audio'], 
                    values=[f"'{self.video_id}'", f"'{chat_id}'", f"'{datetime.now()}'"], 
                    table='users')
